...
import type { Api, AssistantMessage, Model, TextContent, ThinkingContent, ToolCall } from "@mariozechner/pi-ai";
...
function extractText(parts: (TextContent | ThinkingContent | ToolCall)[]): string {
	return parts
...
function collectCustomMessages(branch: SessionEntry[]): string {
	...
	for (let i = branch.length - 1; i >= 0; i--) {
		...
		if (entry.message.role === "custom") {
			msgs.unshift(`[${entry.message.customType}] ${entry.message.content ?? ""}`);
		}
	}
	return msgs.join("\n");
}
...
async function classifyPrompt(
	...
	if (!auth.ok) throw new Error(`No API key for ${monitor.classify.model}: ${auth.error}`);
	if (!auth.apiKey) throw new Error(`No API key for ${monitor.classify.model}`);
